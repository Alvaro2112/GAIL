...
class Discriminator(nn.Module):
    def __init__(self, env, net_dim, lr, update_batch_size, activation, betas):
        ...
        self.l1 = nn.Linear(env.get_state_dim() + 1, net_dim[0])
    ...
    def update(self, pool, data):
        ...
        for learner_ob_b, learner_ac_b, _, _, _, _ in learner_iter:
            ...
            trajs = torch.cat([learner_ob_b, learner_ac_b.unsqueeze(1).float()], 1)
            learner_prob = self(trajs)

            expert_trajs = torch.cat(
                [torch.FloatTensor(expert_ob_b).squeeze(1), torch.FloatTensor(expert_ac_b).unsqueeze(1)], 1)
