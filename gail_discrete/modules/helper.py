...
def get_reward(observation, action, discriminator):
    ...
    traj = torch.cat([observation, action_tensor], 1).squeeze(0)
...
def initiate_run(env_name, actor_net_dim, critic_net_dim, dsicriminator_net_dim,
    ...
    if record:
        neptune.create_experiment(
            params={'lr': lr, 'gamma': gamma, 'lambda': tau, 'grad_clip': grad_clip, 'entropy_weight': entropy_weight,
                    ...
                    'activation': activation, 'actor_net_dim[0]': actor_net_dim[0], 'actor_net_dim[1]': actor_net_dim[1],
    ...
    if env_name == "LunarLander-v2":
        expert = './ppo/policies/lunar_lander_' + str(expert) + '.pkl'

    if env_name == "Acrobot-v1":
        expert = './ppo/policies/acrobot_' + str(expert) + '.pkl'

    if env_name == "CartPole-v0":
        expert = './ppo/policies/cart_pole_' + str(expert) + '.pkl'
    if env_name == "MountainCar-v0" or env_name == "SparseMountainCar-v0":
        expert = './ppo/policies/mountain_car_' + str(expert) + '.pkl'
